...
void *mi_scene_edit	(miTag	tag);
void mi_scene_edit_end	(miTag	tag);
void *mi_scene_growedit	(miTag	tag, ...);
struct miState;
...
struct miInstance {
	...
	miUint1 	spare2;			/* not used */
	miUint1 	shadowmap; 		/* casts shadowmap shadows? */
	miCBoolean	exclusive;		/* light list is exclusive*/

	miTag		light_list;		/* list of instance light tags */

	miUint1		finalgather_force;	/* FG force mode on? */
	miUint1		spare1;			/* not used */
	miCBoolean	builtin;		/* internally generated, do not echo */
	miCBoolean	shadow_excl;		/* shadow list is exclusive */

	miTag		shadow_list;		/* list of shadow light tags */
	miScalar	shading_samples;	/* rasterizer: tesselation */
	miScalar	sample_limits[2];	/* unified sampling limits */
	miTag		time_motion_transform;	/* extended motion transformation */
	miTag           attributes;		/* attributes (SCENE::Options) */
...
typedef struct miGroup {
	...
	miTag		userdata;	/* optional user data blocks */
	miUint		label;		/* optional label */
        miTag		attributes;	/* custom attributes */
...
typedef struct miLight {
	...
	miTag			emitter_spectrum; /* for spectral rendering */
	miScalar		shadowmap_bias;	/* move shmap Z back by bias */
	struct miLight_shmap	shmap;		/* shadowmap data */
	miTag			lightprofile;	/* light profile to be used */
	miCBoolean		use_lprof;	/* true: lambertian or lprof */
	miCBoolean		photons_only;	/* true: photons stored at first hit*/
	miCBoolean		builtin_ol;	/* true: builtin object light */
	miCBoolean		spare1;
	int			spare4[8];
} miLight;
...
struct miMaterial {
	...
	miTag		spare;			/* not used */
	miUint2		builtin_index;		/* builtin shader index */
	miUint2		spare1;			/* not used */
	miTag		bsdf;			/* BSDF 'shader' */
        miUint4         spare2;              	/* reserved for future use */
};
...
typedef struct {			/* parameters[] if miFUNCTION_OUTFILE*/
	enum miImg_format format; 	/* format or miIMG_FORMAT_PS */
	enum miImg_type	type;		/* Not used any more. PHEN deduces it. */
 ...
 * mind that pointers may be 64 bits. miTYPE_TEX is a catchall for texture
 * map values; when the parser finds a map value it doesn't know yet which
 * type of texture map. This is not to be confused with miTYPE_MAP, which
 * is the type of the generic map container.
 */

typedef enum miParam_type {
	miTYPE_INVALID = -1,
	...
	miTYPE_SPECTRUM,			/* spectrum rep as args */
	miTYPE_MAP,
	miNTYPES
} miParam_type;
...
#define miAPPLY_COLORPROFILE	(1 << miSHADER_COLORPROFILE)
...
typedef struct miPhen_decl {
	...
	miSchar 	volume_level;		/* optional volume level */
	...
	miCBoolean	spare3;
	miTag		extd_info;		/* extended data */
	miCBoolean	spare2[4];
        miTag           mdl_module_tag;         /* Mdl_module, if MDL material/function */
        miCBoolean      autovolume;             /* needs autovolume mode ? */
        miCBoolean      photon_autovolume;      /* needs photon autovolume mode ? */
        miCBoolean      spare7;
        miCBoolean	spare8;
} miPhen_decl;
...
typedef struct miFunction_decl {
	miPointer	sparep;
        miTag		external_names;         /* metasl parameter map */
        miTag	        shader_builder;		/* metasl shader builder tag */
	...
	miUint		spare2;			/* obsolete */
	miCBoolean	builtin_rcq;		/* builtin shader (RCQ) */
	miCBoolean	builtin_gen;		/* builtin, but fully linked */
 ...
 *****************************************************************************
 *
 * Element type:    miSCENE_MAP_DECL
 * Data type:	    miMap_decl
 * Sizes:
 * Defaults:	    all nulls except sizes
 */

typedef struct miMap_decl {
	miPointer	sparep[2];
	int		declaration_size;	/* size of declaration */
	char		name[miNAMESIZE];	/* ascii name */
	miUint		dimension;		/* map dimension */
	char		declaration[8]; 	/* declaration string */
} miMap_decl;
 ...
 * Data type:	    miGeoBox
 * Sizes:	    int nvec, int nvert, int npri,
 *		    miBoolean moving
 * Defaults:	    all nulls except sizes
 */
...
/*:: miGeoBox has been replaced by miBox.
     miGeoBox is used by GAP internally and
     some obsolete interface routines. ::*/

typedef struct miGeoBox {
    	miPointer	        pinfo;		/* primdata info */
	miPointer	        primdata;	/* primdata */
	miUint		        pinfo_size;	/* primdata info size (# int) */
	miUint		        primdata_size;	/* primdata size (# int) */
	...
	miUint1 	        spare5;         /* not used */
	miUint1 	        sharp;		/* 0=smooth normals, 255=faceted */
	miCBoolean	        spare3[4];	/* not used */
	miTag	        	userdata;	/* optional user data blocks */
	miTag       		facedata;	/* optional data for triangles */
	miUint	        	spare4;		/* not used */
	miTag		        intersect;	/* intersection calculation */
	struct miVertex_content vert_info;	/* size  content of vertices */
	struct miGeoVector_list vect_info;	/* sections of vector array */
	...
	miVector	        bbox_max;	/* bounding box: high corner */
        char                    pad[4];         /* pad to a multiple of 8 bytes */
...
#define miCPROF_BOXRGB		0x0c	/* R >= 575nm, B <= 500nm, else G    */
...
typedef struct miColor_profile {
	...
	miScalar	inv_trafo[9];	/* color space to CIE XYZ trafo */
        miTag		shader;		/* custom colorpofile shader */
	miUint		spare[2];
} miColor_profile;
...
typedef struct miLight_profile {
	...
	miScalar        integral;       /* integral over all directions */
	miInteger       spare[3];
	miScalar	table[1];
} miLight_profile;
 ...
 *****************************************************************************
 *
 * Element type:    miSCENE_METASL
 * Data type:	    miMetasl
 * Sizes:	    ---
 * Defaults:	    see documentation
 */

typedef struct miMetasl {
	miTag		filename;	/* optional metasl code filename */
	miTag		code;		/* optional metasl code string */
	miUint		spare[6];	/* for the future use */
} miMetasl;
 ...
 *****************************************************************************
 *
 * Element type:    miSCENE_ASSEMBLY
 * Data type:	    miAssembly
 * Sizes:	    ---
 * Defaults:	    see documentation
 */
...
struct miAssembly {
	...
	miPointer	arguments;
	miTag		filename;
	miPointer	delete_cb;	/* item delete callback */
};
...
enum miObject_type {
	...
	miOBJECT_FACES			= 1,
	miOBJECT_BOXES			= 2,
	miOBJECT_SPACECURVES		= 3,
	miOBJECT_SUBDIVSURFS		= 4,
	miOBJECT_PLACEHOLDER		= 6,	/* read geometry on demand */
	miOBJECT_HAIR			= 7,
        miOBJECT_UNUSED                 = 8,	/* unused */
	miOBJECT_MAP			= 9
};
...
typedef struct miPolygon_list {
	...
	struct miGeoVector_list vect_info;		/* contents of vector array */
	struct miVertex_content vert_info;		/* vertex size	content */
...
#define PTEX_REMAP_CHILD_MASK	0xc0000000
#define PTEX_REMAP_CHILD_BIT	30
#define PTEX_REMAP_BQUAD	(1 << 29)
typedef struct miSubdivsurf_list {
	miGeoIndex	no_subdivsurfs;
	miTag		subdivsurfs;
	miTag		userdata;
	miTag		face_userdata;
	miUint		face_userdata_size;	/* size per entry (polygon) */
	miTag		ptex_remap;
	miUint		spare[2];
} miSubdivsurf_list;
...
typedef struct miHair_list {
	...
	miInteger	max_scalars; 		/* max no scalars per hair */
	...
	int		space_max_depth;	/* space subdiv. tree depth */
        miApprox        approx;                 /* approx technique for hair */
        miUint4         spare1[2];              /* reserved for future use */
} miHair_list;
...
struct miObject {
	miTag			photonmap_file;	/* obj. pmap file name,if any*/
	...
	miUint1 		spare_2; 	/* not used */
	char			face;		/* a=all, f=front, b=back    */
	miCBoolean		own_accel;	/* get its own accel. struct?*/
	miUint1			finalgather_force; /* FG force mode on?      */
	miUint1			spare_1[3];	/* not used */
	...
	miScalar		shading_samples;/* rasterizer: tesselation   */
	miUint			tri_est;	/* expected approx #triangles*/
        miTag                   attributes;     /* custom attributes         */
	miTag			vertex_info;	/* vertex name info */
...
typedef struct miSub_object {
	...
	miUint1 	spare2[2];
        miUint4         n_tri;          /* number of triangles */
        miUint4         spare3[3];      /* reserved for future use */
} miSub_object;
...
typedef struct miSplit_object {
	...
	miTag		shared_data;	/* shared flushable data for subobj. */
        miUint4         spare[3];       /* reserved for future use */
...
typedef struct miFace {
	...
	struct miVertex_content gap_vert_info;   /* For calculating box sizes */
} miFace;
...
typedef struct miSpacecurve {
	struct miApprox	        def_approx;	/* default approximation */
	miGeoIndex	        no_curves;	/* total number of curves */
	miGeoIndex	        curve_idx;	/* index into	the 'curves' list*/
	miBoolean	        pad;		/* not used */
	struct miVertex_content gap_vert_info; /* For calculating box sizes */
...
typedef struct miVertex_info {
	...
	miUchar		spare[3];	/* not used */
        miUint4         spare2[2];      /* reserved for future use */
} miVertex_info;
...
#define miSCENE_PRIMLIST_TYPE_MASK	0xffffffff
...
typedef struct miBox {
	miUint8		prim_offset;	/* byte offset: primitives */
	miUint8		mtl_offset;	/* byte offset: materials */
	miUint8		dim_offset;	/* byte offset: tex/usr offset table*/
	miUint8		pdata_offset;	/* byte offset: prim data */
	miUint8		pinfo_offset;	/* byte offset: prim data info */
	...
	miUint		pdata_size;	/* prim data: total size;
					   (for compatibility) */
	miUint		pinfo_size;	/* prim data info size;
					   (extra data can be stored here) */
...
#define miBOX_VERTEX_LINE(box, idx)	\
	((box)->data + (miUint8)(idx) * (miUint8)(box)->vertex_info.line_size)
...
#define miBOX_PD_INFO(box)	\
	(miUint*)(((char*)(box))+(box)->pinfo_offset)

#define miBOX_PD_INFO_SIZE(box)	\
	(box)->pinfo_size
...
#define miBOX_SIZE(box)			\
	(miUint8)((box)->pinfo_offset+(box)->pinfo_size)
...
/* Return address of first border primitive data.
   assumes there is a primdata set for every prim. */
#define miBOX_BORDER_PRIMDATA(box)         \
	(miBOX_PRIMDATA(box) + (box)->no_prims*(box)->primdata_size)

/* Return primitve data for a border triangle. */
#define miBOX_BORDER_PD(box, idx)  \
        (miBOX_BORDER_PRIMDATA(box) + (idx)*(box)->primdata_size)
...
typedef struct miVref {			/* dlist data type for miDLIST_VREF */
	...
	miGeoScalar	w;		/* homogeneous coordinate */
} miVref;
...
typedef enum miApi_vertexflags {
	miAPI_V_SMOOTH=0,
	miAPI_V_CORNER,
	miAPI_V_CONIC,
	miAPI_V_CUSP,
...
/*
 * Polygon count specification for ccmesh.
 */

typedef struct miApi_ccmesh_options {
	miUint	no_polygons;		/* number of polygons */
	miUint	no_vertices;		/* number of vertices */
	miUint	data_size;
	miUint	fdata_size;
	miUint *data;
	miUint *fdata;
} miApi_ccmesh_options;
...
typedef miBoolean (*miApi_object_placeholder_callback)
                  (miTag, miObject *, void *);
typedef miBoolean (*miApi_item_delete_callback)
					(miTag item_tag, void * args);
...
miBoolean mi_api_rename			(char *newname, char *oldname);
...
char     *mi_api_registry_eval		(char *);
char     *mi_api_registry_lookup	(char *);
...
struct miOptions *mi_api_options_begin(char *);
...
miBoolean mi_api_render_params		(miTag*, miTag*, miTag*, miTag*, miInh_func*);
...
miBoolean mi_api_texture_array_typed_def_begin(int, int, char*);
...
miTag     mi_api_texture_fileext_def	(char *, char *);
...
miTag 	  mi_api_texture_callback_def_x(miApi_texture_callback,
				      miApi_texture_callback,
				      miApi_item_delete_callback,
				      void *);
...
miBoolean mi_api_instance_approx	(struct miApprox *, miBoolean);
miBoolean mi_api_instance_register	(miTag, char*);
void	  mi_api_light_list_begin   (void);
miBoolean mi_api_light_list_add     (char*);
miTag     mi_api_light_list_end     (void);
...
miBoolean mi_api_output_file_dir	(char *);

/* The following two functions are deprecated. Superseded by Framebuffer class*/
...
miBoolean mi_api_object_placeholder_callback
					 ...
					 miApi_object_placeholder_callback,
					 void *);
miBoolean mi_api_object_placeholder_callback_x
                                        (miApi_object_placeholder_callback,
					 miApi_object_placeholder_callback,
					 miApi_item_delete_callback,
					 void *);
...
typedef enum miApi_vertexinfo_type {
    miAPI_VERTEXINFO_TYPE_SCALAR, 
    miAPI_VERTEXINFO_TYPE_INTEGER, 
    miAPI_VERTEXINFO_TYPE_VECTOR3, 
    miAPI_VERTEXINFO_TYPE_COLOR
}  miApi_vertexinfo_type;   

typedef struct miApi_vertexinfo {
    miUint			kind;			/* 't' or 'u' */
    miApi_vertexinfo_type	type;
    miUint			dimension;
    const char* 		name;
    miUint			index;			/* for textures, index in state */
} miApi_vertexinfo;
miBoolean mi_api_vertex_info_add	(miApi_vertexinfo *);
...
miBoolean mi_api_poly_approx		(struct miApprox *);
...
miBoolean mi_api_subdivsurf_approx	(char *, struct miApprox *);
miBoolean mi_api_subdivsurf_approx_displace(char *, struct miApprox *);
miBoolean mi_api_subdivsurf_derivative	(int d, int space);
miBoolean mi_api_subdivsurf_texspace	(miApi_texspace_options *,
					 miUint no_spaces);

						/*----------- apiccmesh.cpp */
miBoolean mi_api_ccmesh_begin		(char *, miApi_ccmesh_options *);
miBoolean mi_api_ccmesh_end		(void);
miBoolean mi_api_ccmesh_polygon		(miUint, miUint *, miUint);
miBoolean mi_api_ccmesh_crease		(miScalar *);
miBoolean mi_api_ccmesh_approx		(char *, struct miApprox *);
miBoolean mi_api_ccmesh_derivative	(miUint, miUint);
miBoolean mi_api_ccmesh_bary		(miBoolean);
...
miBoolean mi_api_surface_approx		(char *, struct miApprox *);
miBoolean mi_api_surface_approx_displace(char *, struct miApprox *);
miBoolean mi_api_surface_approx_trim	(char *, struct miApprox *);
...
void	 *mi_api_curve_lookup		(char *, enum miCurve_type, miBoolean,
					 struct miGeoRange *, int *);
miBoolean mi_api_curve_approx		(char *, struct miApprox *);
miBoolean mi_api_curve_approx_store	(miDlist *, struct miApprox *);

						/*------ apispacecurve.cpp */
miBoolean mi_api_spacecurve_begin	(char *);
miBoolean mi_api_spacecurve_curveseg	(miBoolean, char *, miGeoRange *);
miBoolean mi_api_spacecurve_approx	(char *, struct miApprox *);
...
typedef char *miParameter;
						/*----------- apideclare.cpp */
struct miFunction_decl *mi_api_funcdecl_begin	(miParameter *, char *, miParameter *);
miTag     mi_api_funcdecl_end		(void);
miParameter *mi_api_parameter_decl	(enum miParam_type, char *, int);
miParameter *mi_api_parameter_append	(miParameter *, miParameter *);
miBoolean mi_api_parameter_default	(enum miParam_type, void *);
miBoolean mi_api_parameter_child	(miParameter *, miParameter *);
miBoolean mi_api_parameter_lookup	(enum miParam_type *, miBoolean *, int *,
					 miTag, miBoolean, char *);
miBoolean mi_api_parameter_path_lookup	(miTag *, int *, int *, enum miParam_type *,
					 char *);
miBoolean mi_api_parameter_offset_lookup(enum miParam_type *, miBoolean *, char
...
miBoolean mi_api_parameter_value	(enum miParam_type, void *, int *, int *);
...
miBoolean mi_api_parameter_colorprofile (char *);
...
miBoolean mi_api_shader_call_x		(miColor *, miTag, char *, char *,
					 void*);
						...
						/*----------- apidata.cpp */
typedef miBoolean (*miApi_data_callback)   (miTag, struct miUserdata *, void *);
...
miBoolean mi_api_data_callback_x	(miApi_data_callback,
					 miApi_data_callback,
					 miApi_item_delete_callback,
					 void *);
...
void mi_api_nodes_begin();
void mi_api_nodes_end();
void mi_api_node_add(char*, char*);
miBoolean mi_api_scope_begin		(char *);
miBoolean mi_api_scope_end		(void);
char	 *mi_api_scope_apply		(char *);
struct miHair_list *mi_api_hair_begin	(void);
...
struct miLight_profile
            *mi_api_lightprofile_begin  (char *);
miTag        mi_api_lightprofile_end    (void);
miTag        mi_api_lightprofile_lookup (char *);
miBoolean    mi_api_mappath_config	(char *, char *, char *);
char	    *mi_api_mappath_search	(char *);
miParameter *mi_api_map_field_decl	(miParam_type, char *, int, miBoolean);
miParameter *mi_api_map_field_append	(miParameter *,	miParameter *);
miBoolean    mi_api_map_decl_dim	(int);
miMap_decl  *mi_api_map_decl_begin	(char *, miParameter *);
miTag        mi_api_map_decl_end	(void);
miBoolean    mi_api_map_begin		(char *);
miBoolean    mi_api_map_global_value	(miParam_type, void *);
miBoolean    mi_api_map_value		(miParam_type, void *);
miBoolean    mi_api_map_field_end	(void);
miBoolean    mi_api_map_global_end	(void);
miBoolean    mi_api_map_element_end	(void);
miBoolean    mi_api_map_file		(char *);
miTag        mi_api_map_end		(miTag);
miBoolean    mi_api_map_add		(char *, miTag);
miTag	     mi_api_map_lookup		(char *);
miBoolean    mi_api_map_obj_type	(char *);
miBoolean    mi_api_map_obj_field	(char *, char *);
miBoolean    mi_api_map_obj_field_set_value(char *, float *);
...
void	     mi_api_colorprofile_gamma  (miScalar	gamma,
					 miScalar	offset,
					 miBoolean	force);
miBoolean    mi_api_colorprofile_custom (struct miColor_profile *cprof,
					 miMatrix	 m);
miBoolean    mi_api_colorprofile_shader (miTag		shader);  
miTag	     mi_api_colorprofile_lookup	(char		*name);
miBoolean    mi_api_colorprofile_to_renderspace(
					 miColor*	col,
					 char*		name,
					 miScalar	r,
					 miScalar	g,
...
struct miGeoBox *mi_api_trilist_begin	(struct miVertex_content*,
					 miUint, miUint, miUint);
miBoolean    mi_api_trilist_approx	(struct miApprox*);
...
struct miBox *mi_api_primlist_begin	(struct miVertex_info*, miUint, miUint,
					 miUint, miUint, miUint, miUint);
struct miBox *mi_api_primlist_begin_2	(struct miVertex_info*, miUint, miUint,
					 miUint, miUint, miUint, miUint,
					 miUint);
...
miBoolean    mi_api_primlist_approx	(struct miApprox*);
miBoolean    mi_api_primlist_end	(void);
char *mi_api_texpath_search		(char *); 	/* resolve texture file name in the texture path list */
struct miState;
typedef miTag (*miApi_assembly_callback)(const struct miState *, void * args);
typedef miBoolean (*miApi_assembly_exit_callback)
					(const struct miState *, void * args);
miBoolean mi_api_assembly_path_config	(char *, char *, char *);
char * mi_api_assembly_path_search	(char *);
struct miAssembly * mi_api_assembly_begin(char * name);
miBoolean mi_api_assembly_filename	(char * filename);
miBoolean mi_api_assembly_callback	(miApi_assembly_callback,
					 miApi_assembly_exit_callback,
					 void * args);
miBoolean mi_api_assembly_callback_x	(miApi_assembly_callback,
					 miApi_assembly_exit_callback,
					 miApi_item_delete_callback,
					 void * args);

miTag mi_api_assembly_end		();
miBoolean mi_api_assembly_root		(char *);
miTag mi_api_get_assembly_root		();
miMetasl* mi_api_metasl_begin		(char *);
miBoolean mi_api_metasl_filename	(char *);
miBoolean mi_api_metasl_code		(char *);
miTag mi_api_metasl_end			(void);
miBoolean mi_api_mdl_load_module        (char *);
miBoolean mi_api_mdl_load_module_from_string
...
typedef struct miGeoshaderEvalObj {
	miGeoVector	*pos;
	miGeoVector	*normal;
	miGeoVector	*dxdu;
	miGeoVector	*dxdv;
...
typedef struct miGeoshaderEvalCurve {
	miGeoVector2d	*uv;
	miGeoVector2d	*dt;
	miGeoVector2d	*dt2;
...
typedef struct miGeoshaderObjectAdjacency {
...
enum {
    miEO_INST_GROUP     = 0x00000001,   /* instance of a group */
    miEO_INST_OBJECT    = 0x00000002,   /* instance of a object/box */
    miEO_INST_CAMERA    = 0x00000004,   /* instance of a camera */
    miEO_INST_LIGHT     = 0x00000008,   /* instance of a light */
    miEO_INST_FUNCTION  = 0x00000010,   /* instance of a geom shader */
    miEO_GROUP          = 0x00000020,   /* group of instances */
    miEO_OBJECT         = 0x00000040,   /* geometric object or box */
    miEO_CAMERA         = 0x00000080,   /* camera */
    miEO_LIGHT          = 0x00000100,   /* light source */
    miEO_MATERIAL       = 0x00000200,   /* material */
    miEO_IMAGE          = 0x00000400,   /* pixel rectangle */
    miEO_OPTIONS        = 0x00000800,   /* options block */
    miEO_FUNCTION       = 0x00001000,   /* shader (dont_echo: named) */
    miEO_FUNCTION_DECL  = 0x00002000,   /* shader declaration */
    miEO_USERDATA       = 0x00004000,   /* user data block */
    miEO_ASSEMBLY       = 0x00008000,   /* assembly */
    miEO_MAP_DECL       = 0x00010000,   /* map declaration */
    miEO_MAP            = 0x00020000,   /* map */
    miEO_GUI		= 0x00080000,	/* gui */
    miEO_COMPRESS_NONE  = 0,
    miEO_COMPRESS_GZIP  = 1
};

typedef struct miEchoOptions {
	miBoolean	prefer_approx_polygons;	/* triangles from polygons */
	miBoolean	prefer_approx_faces;	/* triangles from surfaces */
	miCBoolean	ascii_output;		/* non-binary output */
	miUchar         compressed_output;      /* compression mode */
	miCBoolean	iray_materials;		/* echo iray materials instead */
        miCBoolean      gui;			/* echo gui arrtibutes */
	...
	miCBoolean	incremental;		/* no re-echo of prev frame */
	miCBoolean	call_geoshader;		/* geometry instead of func-call? */
	miCBoolean	call_assembly;		/* inline assembly instead of ref */
	miCBoolean	spare;			/* for future use */
} miEchoOptions;
