...
static const int mi_ray_interface_version = 2;
...
extern "C" mi::shader::Interface *mi_get_shader_interface(
...
struct Framebuffer {
    public:
        ...
        /// \param count is the number of buffers.
...
/// \brief Implementation abstract base class.
///
/// Do not use it directly, use Access_fb instead
class Access_fb_prim {
    public:
        virtual const Framebuffer * operator->() = 0;
...
/// \brief Implementation abstract base class.
///
/// Do not use it directly, use Edit_fb instead

class Edit_fb_prim {
    public:
        virtual Framebuffer * operator->() = 0;
        virtual void release() = 0;
};
...
struct Interface {
    ...
    /// /return pointer to the internal reference to the framebuffer class
    virtual Access_fb_prim *accessFramebuffer(
	miTag 		buffer_tag);
    ...
    /// /param buffer_tag database tag for the framebuffer collection
    /// /return pointer to the internal reference to the framebuffer class
    virtual Edit_fb_prim *editFramebuffer(
	miTag 		buffer_tag);
...
public:
    ...
    LightIterator(				      	     //!< use to create iter within shader
	miState* 	state,				     //!< the state
	const miVector	&axis,			      	     //!< the axis on the cone for light sample dorections
	miScalar	spread_cos,                   	     //!< the cosine of the light spread angle
	miTag*	 	shader_light_list = 0,		     //!< optional array of light tags
	int	 	n_shader_lights = 0,		     //!< number of lights in above array
        int      	version = mi_ray_interface_version); //!< the ray interface version
...
{
    mi::shader::Interface *iface = mi_get_shader_interface();
    m_afb = iface->accessFramebuffer(tag);
    iface->release();
    iface = 0;
}
...
inline const Framebuffer * Access_fb::operator->()
{
    if(m_afb != 0){
	return (*m_afb).operator->();
    }
    return 0;
}
...
{
    mi::shader::Interface *iface = mi_get_shader_interface();
    m_efb = iface->editFramebuffer(tag);
    iface->release();
    iface = 0;
}
...
inline Framebuffer * Edit_fb::operator->()
{
    if(m_efb != 0){
	return (*m_efb).operator->();
    }
    return 0;
}
