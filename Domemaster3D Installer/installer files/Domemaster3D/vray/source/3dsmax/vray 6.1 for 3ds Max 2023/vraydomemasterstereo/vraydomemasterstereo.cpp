...
public:
  ...
  float GetTDist(TimeValue t, Interval &valid=FOREVER) override { return 100.0f; }
...
static ParamBlockDesc2 camera_param_blk(camera_params, loadStringResource(idtr_grp_params, false), 0, &cameraClassDesc,
...
int VRayCamera::getScreenRay(
  ...
  //VR::ShadeVec dir = getDir(xs, ys, GETDIR);   //Return the dir data from the getDir function
  ...
  //VR::ShadeVec org = getDir(xs, ys, GETORG);   //Return the org data from the getDir function
  ...
  const VR::VRayFrameData &fdata = vray->getFrameData();
  ray.p = fdata.camToWorld.offs + fdata.camToWorld.m * VR::toShadeVec(org);
  ray.dir = fdata.camToWorld.m * VR::toShadeVec(dir);
