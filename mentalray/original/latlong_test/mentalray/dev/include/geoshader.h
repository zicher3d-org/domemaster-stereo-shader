...
typedef struct miFunction_decl {
	...
	miCBoolean	builtin;		/* builtin shader */
...
enum miObject_type {
	...
	miOBJECT_HAIR,
	miOBJECT_INTERSECT_FUNC			/* custom type */
};
...
typedef struct miSub_object {
	...
	miUint1 	spare2[2];
        miUint4         spare3[2];      /* reserved for future use */
} miSub_object;
...
typedef struct miBox {
	...
	miUint		pinfo_offset;	/* byte offset: prim data info */
        miUint		spare2[4];
...
#define miBOX_SIZE(box)			\
	(miUint)((box)->pinfo_offset+(box)->pinfo_size)
...
struct miRc_options *mi_api_options_begin(char *);
...
#define miEO_ASSEMBLY	   0x00008000		/* assembly */
