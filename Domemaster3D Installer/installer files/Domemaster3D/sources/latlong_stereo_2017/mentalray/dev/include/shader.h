...
#ifdef __cplusplus
#include <cstdio>
#include <cmath>  /* for math function overloading */
#else
#include <stdio.h>
#include <math.h>
#endif
...
#ifdef _WIN32
typedef signed __int64			miSint8;
typedef unsigned __int64		miUint8;
#else
typedef long long			miSint8;
typedef unsigned long long		miUint8;
#endif
...
struct miVector2d { miScalar u, v; };
struct miVector { miScalar x, y, z; };
struct miGeoVector2d { miGeoScalar u, v; };
struct miGeoVector { miGeoScalar x, y, z; };
struct miQuaternion { miScalar x, y, z, w; };
struct miColor { miScalar r, g, b, a; };
struct miRange {miScalar min, max; };
struct miGeoRange {miGeoScalar min, max; };
union miPointer {void *p; miUint4 padding[2];};	/* 8-byte aligned ptr*/
union miLock { struct miLock_struct* milockptr; int padding[2]; };

typedef struct miState		miState;
typedef struct miOptions	miOptions;
typedef struct miImg_image	miImg_image;
typedef struct miInfo		miInfo;
typedef struct miInstance       miInstance;
typedef struct miCamera         miCamera;
typedef struct miObject         miObject;
typedef struct miMaterial       miMaterial;
typedef struct miAssembly	miAssembly;

typedef struct miVector2d 	miVector2d;
typedef struct miVector		miVector;
typedef struct miGeoVector2d	miGeoVector2d;
typedef struct miGeoVector	miGeoVector;
typedef struct miQuaternion	miQuaternion;
typedef struct miColor		miColor;
typedef struct miSpectrum	miSpectrum;
typedef struct miRange		miRange;
typedef struct miGeoRange	miGeoRange;
typedef union miPointer		miPointer;
typedef union miLock		miLock;

typedef int (*miInh_func)(void **, void *, 
	int, void *, int, miInstance *, 
	miInstance *, miMatrix, miMaterial *);



/* int type wide enough for a pointer. */
typedef size_t				miIntptr;
...
#define miNOLIGHTS                      ((miTag) ~0)
#define miGHOSTTAG			((miTag) ~0)
#define miPARAMTAG			((miTag) ~1) /* parameter set directly */

typedef enum miModule {
	...
	miM_RCRM,		/* rendering core: rasterizer */
	...
	miM_RCLM,		/* light mapping module */
        miM_SHDR,               /* shader call */
	miM_RCH,                /* Hair rendering */
	miM_GEOSI = miM_RCH+2,  /* surface / surface intersection */
	miM_LINK  = miM_GEOSI+2,/* dynamical linking, function symbol table */
	...
	miM_PHCR = miM_API + 2,	/* shader editor GUI */
	miM_RCB,		/* RC boxing */
	miM_RCC,		/* RC contours */
	miM_RCI,		/* RC intersection */
	miM_RCGI,		/* RC global illumination and caustics */
	miM_SIMP = miM_RCGI+2,	/* Mesh simplification */
	...
	miM_BSP2,		/* BSP2 */
	miM_DBN,		/* DBN module for neuray db elements */
	miM_BRDF = miM_DBN + 2, /* BRDF routines */
	miM_MATH,		/* MATH module */
	miM_BVH,		/* bounding volume module */
	miM_N2SD,		/* nurbs import */ 
	miM_PARTICLEMAP,	/* particle maps */
	miM_SCHED,		/* interface to neuray scheduler */
	miM_SERIAL,		/* interface to neuray serializer */
	miM_EXTERN,		/* external library routine, like png, expat */ 
	miM_CONT,		/* additional C++ containers */
	miM_SAMP,		/* sampling routines */
	miM_MSL,		/* metasl library */
	miM_DISK = miM_MSL + 2,	/* disk routines */
	miM_MRMSL,		/* mental ray metasl proxy */
	miM_SHADERDB = miM_MRMSL + 2,	/* metasl shader database */
	miM_MSLC,		/* metasl compiler proxy */
	miM_COMP = miM_MSLC + 3,/* metasl compiler kernel */
        miM_MDL,                /* MDL */
	miM_HALF = miM_MDL + 3, /* 16-bit float type */
	miM_RCMAP,		/* RC map */
	miM_LLVM,		/* LLVM MetaSL shaders backend */
...
#endif
#define RAY2			/* defined for mental ray 2.0 and up */
#define RAY3			/* defined for mental ray 3.0 and up */
#define RAY31                   /* defined for mental ray 3.1 and up */
#define RAY32                   /* defined for mental ray 3.2 and up */
#define RAY33                   /* defined for mental ray 3.3 and up */
#define RAY34                   /* defined for mental ray 3.4 and up */
#define RAY35                   /* defined for mental ray 3.5 and up */
#define RAY36                   /* defined for mental ray 3.6 and up */
#define RAY37                   /* defined for mental ray 3.7 and up */
#define RAY38                   /* defined for mental ray 3.8 and up */
#define RAY39                   /* defined for mental ray 3.9 and up */
#define RAY310		        /* defined for mental ray 3.10 and up */
#define RAY311		        /* defined for mental ray 3.11 and up */
#define RAY312		        /* defined for mental ray 3.12 and up */
#define RAY313		        /* defined for mental ray 3.13 and up */
#define RAY314		        /* defined for mental ray 3.14 and up */
...
#define mi_vector_normalize(r)	do { miScalar _n = mi_vector_norm(r);\
				     mi_vector_div(r, _n); } while (0)

#define mi_vector_normalize_d(r) do { double _n= mi_vector_norm_d(r);\
...
miBoolean	mi_matrix_solve		(miScalar *, miMatrix const,
					 miScalar const *, int);
miScalar	mi_spline		(miScalar, int, const miScalar *);
miScalar	mi_random		(void);
miScalar	mi_erandom		(unsigned short seed[3]);
void		mi_srandom		(long newseed);
miScalar	mi_noise_1d		(miScalar);
miScalar	mi_unoise_1d		(miScalar);
miScalar	mi_noise_2d		(miScalar, miScalar);
miScalar	mi_unoise_2d		(miScalar, miScalar);
miScalar	mi_noise_3d		(miVector *);
miScalar	mi_unoise_3d		(const miVector *);
miScalar	mi_noise_1d_grad	(miScalar, miScalar *);
miScalar	mi_unoise_1d_grad	(miScalar, miScalar *);
miScalar	mi_noise_2d_grad	(miScalar, miScalar,
					 miScalar *, miScalar *);
miScalar	mi_unoise_2d_grad	(miScalar, miScalar,
					 miScalar *, miScalar *);
miScalar	mi_noise_3d_grad	(const miVector *, miVector *);
miScalar	mi_unoise_3d_grad	(const miVector *, miVector *);
...
void		mi_fatal		(const char *, ...);
void		mi_error		(const char *, ...);
void		mi_warning		(const char *, ...);
void		mi_info			(const char *, ...);
void		mi_progress		(const char *, ...);
void		mi_debug		(const char *, ...);
void		mi_vdebug		(const char *, ...);
void		mi_nfatal		(int, const char *, ...);
void		mi_nerror		(int, const char *, ...);
void		mi_nwarning		(int, const char *, ...);
const char	*mi_strerror		(void);

char *mi_string_substitute(	/* apply dos/unix etc path substns */
	char		*new_name,		/* place to copy mangled path*/
	const char	*name,			/* path to be mangled */
...
miScalar mi_blackbody_spectral_radiance( /* in W/(sr m^2 nm) */
    miScalar temperature, /* in Kelvin */
    miScalar wavelength); /* in nano-meter */

miScalar mi_blackbody_radiance( /* W/m^2/sr */
    miColor* c, miScalar temperature, miBoolean normalized);

void mi_ciexyz_weights(miColor* weight, miScalar wavelen);


/* returned by miQ_PRI_DATA: */
typedef struct miPri_data {
	miUint		info_size;	/* size of info */
	miUint		*info;		/* info: data layout */
	miUint		data_size;	/* size of triangle data */
	miUint		*data;		/* primitive (triangle) data */
} miPri_data;

typedef enum miQ_type {
	miQ_NONE			= 0,
	miQ_VERSION			= 1,
	miQ_DATE			= 2,
	miQ_INST_FUNCTION		= 3,
	...
	miQ_LIGHT_LABEL			= 98,
	miQ_LIGHT_DATA			= 99,
	miQ_LIGHT_TANGENT_SPACE		= 155,
	miQ_LIGHT_NORMAL                = 156,
	...
	miQ_PRI_DATA			= 154,
	miQ_PRI_INDEX			= 159,
	miQ_SAMPLE_MOVING		= 158,
	miQ_RAY_INTERFACE               = 157,
	miQ_STAGE		        = 161,
	miQ_BUMP_BASIS		        = 163,
        miQ_HAIR_NUM_TEXTURES           = 182,
	miQ_HAIR_NUM_USERS              = 166,
        miQ_HAIR_USERS                  = 167,
        miQ_HAIR_RADIUS                 = 113,
	miQ_PRI_NUM_VERTICES		= 181,
	miQ_PRI_VERTICES_POINTS		= 168,
	miQ_PRI_VERTICES_NORMALS	= 169,
	miQ_PRI_VERTICES_MOTION		= 170,
	miQ_PRI_VERTICES_TEX		= 171,
	miQ_PRI_VERTICES_USER		= 172,
	miQ_PRI_VERTICES_BUMPS_U	= 173,
	miQ_PRI_VERTICES_BUMPS_V	= 174,
	miQ_PRI_VERTICES_DERIVS_U	= 175,
	miQ_PRI_VERTICES_DERIVS_V	= 176,
	miQ_PRI_VERTICES_DERIVS_UU	= 177,
	miQ_PRI_VERTICES_DERIVS_VV	= 178,
	miQ_PRI_VERTICES_DERIVS_UV	= 179,
	miQ_PRI_VERTICES_DERIVS_VU	= miQ_PRI_VERTICES_DERIVS_UV,
	miQ_PRI_PARTICLE_INDEX		= 180,
	miQ_PRI_PARTICLE_TYPE		= 184,
	miQ_PRI_PARTICLE_UV		= 185,
	miQ_PRI_SDS_UV			= 186,
	miQ_PRI_UV_DIFFERENTIALS	= 187,
	miQ_PRI_NUM_DATA		= 188,
	miQ_NUM_USERS			= 189,
	miQ_USER_DIM			= 190,
	miQ_PRI_VERTEX_INFO		= 191,
	miQ_PRI_VERTEX_INFO_INDEX	= 192,
	miQ_PRI_NUM_VERTEX_INFOS	= 193,
	miQ_PRI_TYPE			= 194,
	miQ_SHADING_CACHEABLE		= 195,
	miNQ				= 196
} miQ_type;

/* returned values for the miQ_STAGE mode */
typedef enum {
	miSTAGE_UNKNOWN			= 0,		/* no rendering/unknown stage */
	miSTAGE_TILE 			= 1 << 0,	/* rendering tile */
	miSTAGE_FINALGATHER_PRECOMP	= 1 << 1,	/* precomputing final gathering */
	miSTAGE_AO_PRECOMP		= 1 << 2,	/* precomputing ambient occlusion */
	miSTAGE_CAUSTIC_EMIT		= 1 << 3,	/* caustic photon emission */
	miSTAGE_GLOBILLUM_EMIT		= 1 << 4,	/* globillum photon emission */
	miSTAGE_PHOTON_EMIT		= miSTAGE_CAUSTIC_EMIT | miSTAGE_GLOBILLUM_EMIT,
	miSTAGE_IMPORTON_EMIT		= 1 << 5,	/* importon emission */
	miSTAGE_LIGHTMAP		= 1 << 6,	/* computing light map */
	miSTAGE_IP_COMP			= 1 << 7,	/* irr particles computation */	
	miSTAGE_IBL_BAKE		= 1 << 8	/* prebaking IBL environment */
} miStage; 

struct miState;
miBoolean	mi_query(miQ_type, miState *, miUint, void *, ...);
...
miTag mi_kdtree_create(
	miKdtree_type	type,		/* access type and dimension */
...
miUint mi_kdtree_lookup(
	...
	miScalar	*max_dist_used,	   /* out: dist. to farthest elem.*/
...
/*
 * Uncomment this to use size_t allocators, but loose binary compatibility
 * with older mental ray versions.
 */
/* 
#define MI_USE_SIZET_ALLOCATORS
*/


#ifndef MI_USE_SIZET_ALLOCATORS
...
#define mi_mem_test_reallocate(m,s) mi_mem_int_test_reallocate(__FILE__,__LINE__,(m),(s))
#else
#define mi_mem_allocate(s)	    mi_mem_int_allocate_sizet  (__FILE__,__LINE__,(s))
#define mi_mem_reallocate(m,s)	    mi_mem_int_reallocate_sizet(__FILE__,__LINE__,(m),(s))
#define mi_mem_test_allocate(s)	    mi_mem_int_test_allocate_sizet(__FILE__,__LINE__,(s))
#define mi_mem_test_reallocate(m,s) mi_mem_int_test_reallocate_sizet(__FILE__,__LINE__,(m),(s))
#endif
...
void		*mi_mem_int_allocate	   	    (const char *, int, miUint);
void		*mi_mem_int_allocate_sizet 	    (const char *, int, size_t);
void		*mi_mem_int_test_allocate  	    (const char *, int, miUint);
void		*mi_mem_int_test_allocate_sizet     (const char *, int, size_t);
void		*mi_mem_int_reallocate	   	    (const char *, int, void *, miUint);
void 		*mi_mem_int_reallocate_sizet	    (const char *, int, void *, size_t);
void		*mi_mem_int_test_reallocate	    (const char *, int, void *, miUint);
void		*mi_mem_int_test_reallocate_sizet   (const char *, int, void *, size_t);
void		 mi_mem_int_release	   	    (const char *, int, void *);
void		 mi_mem_check		(void);
miUlong		 mi_mem_getsize		(void);
void		 mi_mem_summary		(void);
void		 mi_mem_dump		(const miModule);

char		*mi_mem_int_strdup	(const char *, int, const char *);
...
extern const int mi_mem_blkmaxsize; /* maximal block size */
miMemblk	*mi_mem_int_blkcreate	(const char *, int, int);
...
const void  *mi_db_access(miTag);	/* access tag content */
int    mi_db_type       (miTag);	/* get tag type */
miUint    mi_db_size    (miTag);	/* check if tag is a job */
miBoolean mi_db_isjob   (miTag);	/* access tag type and content */
void  *mi_db_access_type(
			int *,		/* returned type, really miScene_type*/
			miTag);		/* tag to be accessed */
void   mi_db_unpin	(miTag);	/* tag to be unpinned */
void  *mi_db_create    (
			miTag *,	/* returned new tag */
			int,		/* miSCENE_* type to store */
			int);		/* size of item in bytes */
void  *mi_db_realloc(
			miTag,		/* tag to be resized */
			miUint);	/* new size of the db item */
void   mi_db_delete	(miTag);	/* tag to be deleted */
miTag  mi_db_copy	(miTag);	/* source tag to duplicate */
void   mi_db_dump	(miModule);/* dump whose entries, 0=all */
 ...
 * type of the file to be created. The use as array indexes is the reason
...
enum miImg_type {
	...
	miIMG_TYPE_RGB		= 2,	/* 3*8	bits, color */
	miIMG_TYPE_RGB_16	= 3,	/* 3*16 bits, color */
...
#define miIMG_USER_MASK		0x7f000000	/* bits 25-30 */
/* number of user FBs packed to the mask */
#define miIMG_USER_N_FB		(miIMG_TYPE_USER_FIRST - miIMG_NTYPES)

typedef enum miImg_type miImg_type;
...
enum miImg_format {
	...
	miIMG_FORMAT_DDS	= 61,   /* Microsoft DDS */
	miIMG_FORMAT_MAP_V1     = 62,   /* .map format of mental ray 3.10 and prior */
	miIMG_NFORMATS		= 63,

	miIMG_FORMAT_FROM_EXT   = 255   /* deduce format from file extension */
};

typedef enum miImg_format miImg_format;
/* The documented error return */
...
typedef struct miImg_line {
	unsigned long long	c[4];		/* indexed by miIMG_* */
...
typedef struct miImg_file {
	...
	int		filesize0;	/* deprecated (4GB limit): if reading, 
	...
	miPointer	layer_name;	/* EXR: which layer to open */
	miUint1		flags;		/* additional flags */
	miCBoolean	stereo;		/* EXR */
        unsigned long long filesize;    /* if reading, size of file in bytes */
	int		spare[8];	/* makes sizeof(miImg_file) 256 bytes */
} miImg_file;
...
struct miImg_image {
    ...
    unsigned long long 	dir[miIMG_DIRSIZE]; /* offs from this to other imgs */
    int		width, height;	/* width and height in pixels */
    int		bits;		/* requested bits per comp, 8/16/32 */
    int		comp;		/* requested components/pixel, 1..4 */
    miUint1	flags;		/* additional flags */
    miUint1	writable;	/* writable texture incl. image format */
    ...
    /* tile remapping data */
    int	tile_pixels;		/* number of pixels in a full res tile */
    int	n_tiles;		/* redundant to image height */
    int	tile_width;		/* tile width */
    int	tile_height;		/* tile height */
    int	n_horz_tiles;		/* number of tiles in a row */
    int n_vert_tiles;		/* number of tiles in a column */
    int	horz_offset;		/* non-positive full-size tile border*/
    int	vert_offset;		/* non-positive full-size tile border*/
    int	spare[4];		/* future use */

    unsigned long long	c[4];		/*	indexed by miIMG_*; 4*height */
...
miUchar     	*mi_img_access(
				...
				int,			/* requested scanline*/
				int);			/* req component 0..3*/
miImg_image	*mi_img_image_alloc(
				miImg_file *);		/* file descriptor */
miImg_type	mi_img_type_identify(
				const char *);		/* type, "rgb" etc */
miImg_format	mi_img_format_identify(
				const char *);		/* format, "rla" etc */
miImg_type	mi_img_best_type(
				miImg_format);		/* format to test */
const char	*mi_img_type_name(
				miImg_type);		/* data type to name */
void		mi_img_type_query(
				miImg_type,		/* type to query */
				int *,			/* ret. # of comp */
				int *,			/* ret. # of bits/c */
				miCBoolean *);		/* ret. comp mask.
							 * set to NULL if not
							 * wanted */
const char	*mi_img_format_name(
				miImg_format);		/* format to name */
...
void		mi_img_clip_color(
				const miImg_image *,	/* image to clip for */
				miColor *);		/* color to clip */

void		mi_img_put_color(
				miImg_image *,		/* image to store in */
				const miColor *,	/* color to store */
				int,			/* X coordinate */
				int);			/* Y coordinate */

void		mi_img_put_scalar(
				miImg_image *,		/* image to store in */
				float,			/* scalar to store */
				int,			/* X coordinate */
				int);			/* Y coordinate */
void		mi_img_put_vector(
				miImg_image *,		/* image to store in */
				const miVector *,	/* vector to store */
				int,			/* X coordinate */
				int);			/* Y coordinate */
...
void		mi_img_put_normal(
				miImg_image *,		/* image to store in */
				const miVector *,	/* normal to store */
				int,			/* X coordinate */
				int);			/* Y coordinate */
...
void		mi_img_get_color(
				const miImg_image *,	/* image to load from */
				miColor *,		/* color to read */
				int,			/* X coordinate */
				int);			/* Y coordinate */
void		mi_img_get_scalar(
				const miImg_image *,	/* image to load from */
				float *,		/* scalar to read */
				int,			/* X coordinate */
				int);			/* Y coordinate */
void		mi_img_get_vector(
				const miImg_image *,	/* image to load from */
				miVector *,		/* vector to read */
				int,			/* X coordinate */
				int);			/* Y coordinate */
...
void		mi_img_get_normal(
				const miImg_image *,	/* image to load from */
				miVector *,		/* normal to read */
				int,			/* X coordinate */
				int);			/* Y coordinate */
void		mi_img_get_label(
				const miImg_image *,	/* image to load from */
				miUint *,		/* returned label */
				int,			/* X coordinate */
				int);			/* Y coordinate */
...
enum miRc_query {				/* for mi_rc_run_query */
	...
	miRCQ_CUDA_DEVICE_PROPERTIES,		/* cuda device properties uses arg as device id*/
	miRCQ_CUDA_MEMORY_STATS			/* cuda device mem stats during rendering */
};
...
struct miState;
miC_LINKAGE void *mi_renderpass_access(	/* for renderpass merge function */
	struct miState	*state,		/* state->pri is sample set array */
	int		pass,		/* access which pass, -1=output */
	int		fb);		/* frame buffer number */

miC_LINKAGE miBoolean mi_renderpass_resolution(
...
miC_LINKAGE miBoolean mi_renderpass_sample_get(
...
miC_LINKAGE miBoolean mi_renderpass_sample_put(
...
miC_LINKAGE miBoolean mi_renderpass_samplerect_black(
	miBoolean	*result,	/* set this to miTRUE if black */
	struct miState	*state,		/* state->cache is sample set array */
	int		x,		/* X subpixel coordinate */
	int		y);		/* Y subpixel coordinate */

miC_LINKAGE miBoolean mi_renderpass_samplerect_flush(
	struct miState	*state,		/* state->cache is sample set array */
 ...
 * for optimization.
 */
miC_LINKAGE miBoolean mi_ray_falloff(
 ...
 */
miC_LINKAGE miBoolean mi_ray_offset(
	const struct miState	*state,		/* state */
...
struct miRc_intersection;

miC_LINKAGE void mi_state_set_pri(
	struct miState		*state,		/* state */
	const struct miRc_intersection	*pri,	/* intersected primitive */
	miUint			pri_idx);	/* primitive index */

/*
 * Apply transformation to ray differentials
 */

miC_LINKAGE void mi_ray_differential_transform(
        struct miState              *state,         /* state */
        miMatrix const              transform);     /* transformation to apply */
...
typedef enum miScene_types {
	...
	miSCENE_ATTRIBUTES,     /* 27: attributes (SCENE::Options) */
	...
	miSCENE_USERDATA,	/* 34: miUserdata */
			        /* 35: unused */
	              		...
	              		/* 38: unused */
	                        /* 39: unused */
	miSCENE_NBOX = 40,	/* 40: new miBox, replaces miGeoBox/miSCENE_BOX*/
	...
	miSCENE_INDEXRANGE, 	/* 43: miIndex_range */
        miSCENE_LIGHTPROFILE,   /* 44: light profiles*/
	                        /* 45: unused */
	miSCENE_LIGHTPROFILE_STD = 46,	/* 46: light profile descr. */
        miSCENE_APPROX_LIST,    /* 47:miApprox array*/
	                        /* 48: unused */
	miSCENE_FBINFO = 49,	/* 49: frame buffer information */
	miSCENE_COLORPROFILE,	/* 50: support color profiles */
	miSCENE_SPECTRUM,	/* 51: support light spectra */
	             		/* 52: unused */
	miSCENE_FLIST = 53,	/* 53: face list */
	miSCENE_ASSEMBLY,	/* 54: assembly */
	miSCENE_MAP_DECL,	/* 55: miMap_decl */
	miSCENE_METASL,		/* 56: metasl code */
	miSCENE_PHEN_EXTDINFO,	/* 57: extended phen data */
                                /* 58--62: unused */
        miSCENE_NEURAY = 63,	/* 63: generic neuray type */
...
typedef struct miApprox {
	...
	miScalar	     grading;	/* min angle bound for graded meshing*/
        miUint4              spare[2];  /* reserved for future use           */
} miApprox;
...
#define miLIGHTMAP_ANYHOST	4


/*
 * First-hit render types.
 */

#define miRENDER_FH_OFF         0
#define miRENDER_FH_SCANLINE    1
#define miRENDER_FH_RAST        'r'

/*
 * Acceleration types.
 */

#define miRENDER_ACCEL_OFF      0
#define miRENDER_ACCEL_BSP      'b'
#define miRENDER_ACCEL_LARGEBSP 'l'
#define miRENDER_ACCEL_BSP2     'n'
#define miRENDER_ACCEL_BVH      'B'

/*
 * miOptions, the rendering options.
 */
...
struct miOptions {
	...
	char		render_space;		/* coordinate space 'o' */
						/* 'o' object space, */
						/* 'c' deprecated, camera space */
	miCBoolean	spare11;		/* unused */
	miCBoolean      spare12;		/* unused */
	...
	miUint		spare2[6];		/* unused */
	miUint		spare13;	        /* unused */
	...
	miUint1		spare6;			/* not used */
	miUint1 	spare7;			/* not used */
	...
	miUint1		fb_virtual;		/* fb mem management */
						...
						/* default: 1 */
	miCBoolean	unified_sampling;	/* is unified sampling on?*/
	miCBoolean	spare3[2];		/* not used */
	miTag		string_options;		/* arbitrary name-value options */

	int		spare[9];		/* not used */
};
typedef enum miCamera_eye {
	miCAMERA_EYE_LEFT	= 0,
	miCAMERA_EYE_CENTER	= 1,
	miCAMERA_EYE_RIGHT	= 2
} miCamera_eye;



typedef enum miCamera_stereo {
	miCAMERA_STEREO_OFF	= 0,
	miCAMERA_STEREO_TOEIN	= 1,
	miCAMERA_STEREO_OFFAXIS	= 2,
	miCAMERA_STEREO_OFFSET	= 3
} miCamera_stereo;
...
struct miCamera {
	...
	miTag		filter;			/* MI::RC::Pixel_filter*/
	float		parallax_distance;	/* stereo fusion distance */
	int		spare[6];		/* not used */
};

struct miFramebuffer;
enum miLight_type {
	miLIGHT_ORIGIN,
	miLIGHT_DIRECTION,
	miLIGHT_SPOT
...
enum miLight_area {
	/* describe geometry of light source */
	miLIGHT_NONE = 0,
	miLIGHT_RECTANGLE,
	miLIGHT_DISC,
	miLIGHT_SPHERE,
...
typedef struct miFunction {
	...
	miCBoolean	has_connections;	/* at least one parameter has a
						   connection.
						   if (func->has_connections)
						   replaces if (func->ghost_offs)
						   as a check for connections,
						   because ghost_offs != 0 for
						   anything but defaults. */
	miCBoolean	spare2[2];		/* not used */
	miCBoolean	no_expl_params; 	/* candidate for indirect par*/
	miCBoolean	spare3[6];		/* not used */
...
typedef enum miRay_type {
	...
	miRAY_COLOR_TO_INTERNAL,	/* colorprofile shader */
	miRAY_COLOR_FROM_INTERNAL,	/* colorprofile shader */
	miRAY_NO_TYPES
} miRay_type;
...
/*
 *
 * Defines interaction of object with different kinds of ray.
 *
 */

typedef enum miTrace_flag {
	miTRACE_FLAG_VISIBLE	    = 1u << 0,	/* 'visible'			*/
	miTRACE_FLAG_SHADOW_CAST    = 1u << 1,	/* cast shadow rays		*/
	miTRACE_FLAG_SHADOW_RECV    = 1u << 2,	/* receive shadow rays		*/
	miTRACE_FLAG_REFLECT_CAST   = 1u << 3,	/* cast reflection rays		*/
	miTRACE_FLAG_REFLECT_RECV   = 1u << 4,	/* receive reflection rays	*/
	miTRACE_FLAG_REFRACT_CAST   = 1u << 5,	/* cast refraction rays		*/
	miTRACE_FLAG_REFRACT_RECV   = 1u << 6,	/* receive refraction rays	*/
	miTRACE_FLAG_TRANSP_CAST    = 1u << 7,	/* cast transparency rays	*/
	miTRACE_FLAG_TRANSP_RECV    = 1u << 8,	/* receive transparency rays	*/
	miTRACE_FLAG_CAUSTIC	    = 1u << 9,	/* interact with caustic photon	*/
	miTRACE_FLAG_GLOBILLUM	    = 1u << 10,	/* interact with glob. photon	*/
	miTRACE_FLAG_FINALGATHER    = 1u << 11	/* interact with final gather	*/
} miTrace_flag;
...
typedef enum miShader_type {
	...
	miSHADER_STATE,    	/* allow user to set-up state */
        ...
        miSHADER_COLORPROFILE,  /* colorprofile shader */
	miSHADER_NO_TYPES
} miShader_type;
...
struct miState {
	...
	const struct miCamera 	*camera;	/* camera */
	const struct miOptions 	*options;	/* options */
	miLock		global_lock;		/* global lock for shaders */
	miUint		qmc_instance;		/* instance of low discrepancy
	...
	miMsl_state	*msl_state;		/* MetaSL companion state */
	void		*spare; 		/* unused */
	...
	struct miRc_intersection * pri;		/* intersection or 0 if none */

	float		importance;		/* current ray importance factor */
...
typedef struct miOutstate {			/* state for output shaders */
...
/** Checks if the miTag t is a non-shader/non-interface tag. */
#define mi_is_nonshader_tag(t) ((t)==miNULLTAG || (t)==miPARAMTAG)

/** Checks if the current shader of miState s has ghosts. */
#define mi_has_ghosts(s) (0!=(s)->shader->ghost_offs)

/** Checks if the current shader of miState s has connections */
#define mi_has_connections(s) (miTRUE==(s)->shader->has_connections)

/** Gets the ghost tag for parameter p of the current shader of miState s.
 * This assumes that mi_has_ghosts(s) is true.
 */
#define mi_get_ghost(s, p) (*((miTag*)((char*)(p)+(s)->shader->ghost_offs)))

/** Checks if the parameter p of the current shader of miState s is connected
 * to a shader.
 */
#define mi_is_connected(s, p) \
       (mi_has_connections(s) \
    && (!mi_is_nonshader_tag(mi_get_ghost((s),(p)))))

/** Checks if the parameter p of the current shader of miState s is connected
 * to a shader or set directly. This means that p is default if
 * mi_is_set(s,p)==miFALSE.
 */
#define mi_is_set(s, p) \
    (mi_has_ghosts(s) && miNULLTAG!=mi_get_ghost((s),(p)))

#define mi_eval(s, p) \
    (!mi_is_connected((s),(p)) ? (void*)(p) : \
    mi_phen_eval((s),(void*)(p)))

/* Like mi_eval except that mi_is_set is assumed to be true */
#define mi_eval_set(s, p) \
    (miPARAMTAG==mi_get_ghost((s),(p)) ? (void*)(p) : \
    mi_phen_eval((s),(void*)(p)))

#define mi_eval_boolean(p)	((miBoolean *)mi_eval(state, (p)))
#define mi_eval_integer(p)	((miInteger *)mi_eval(state, (p)))
#define mi_eval_scalar(p)	((miScalar  *)mi_eval(state, (p)))
#define mi_eval_vector(p)	((miVector  *)mi_eval(state, (p)))
#define mi_eval_transform(p)	((miScalar  *)mi_eval(state, (p)))
#define mi_eval_color(p)	((miColor   *)mi_eval(state, (p)))
#define mi_eval_tag(p)		((miTag     *)mi_eval(state, (p)))
#define mi_eval_spectrum(p)     \
    mi_spectrum_ensure((miSpectrum*)mi_eval(state, (p)))

#define mi_call( s, p ) \
    (!mi_is_connected((s), (p)) ? (void*)(p) : \
    mi_phen_call(s,(void*)(p)))

#define mi_call_boolean(p)	((miBoolean *)mi_call(state, (p)))
#define mi_call_integer(p)	((miInteger *)mi_call(state, (p)))
#define mi_call_scalar(p)	((miScalar  *)mi_call(state, (p)))
#define mi_call_vector(p)	((miVector  *)mi_call(state, (p)))
#define mi_call_transform(p)	((miScalar  *)mi_call(state, (p)))
#define mi_call_color(p)	((miColor   *)mi_call(state, (p)))
#define mi_call_tag(p)		((miTag     *)mi_call(state, (p)))
...
void *mi_phen_eval(				/* evaluate a shader param */
                miState		*state, 	/* state */
                void		*parm );	/* parameter */
void *mi_phen_call(				/* re-evaluate a shader param */
                miState		*state, 	/* state */
                void		*parm );	/* parameter */
miBoolean mi_lookup_color_texture(
                miColor* 	color,		/* returned color */
                miState* 	state,
                miTag		tex,		/* texture to look up */
                miVector* 	coord);		/* texture coordinate */
miBoolean mi_lookup_scalar_texture(
                miScalar* 	scalar,		/* returned scalar */
                miState* 	state,
                miTag		tex,		/* texture to look up */
                miVector* 	coord);		/* texture coordinate */
miBoolean mi_lookup_vector_texture(
                miVector*	rvector,	/* returned vector */
                miState* 	state,
                miTag		tex,		/* texture to look up */
                miVector* 	coord);		/* texture coordinate */
miBoolean mi_lookup_color_texture_x(
                ...
                miTag		tex,		/* texture to look up */
                miVector* 	coord,		/* texture coordinate */
                miScalar	filter);	/* filter for lookup */
miBoolean mi_lookup_scalar_texture_x(
                ...
                miVector* 	coord,		/* texture coordinate */
                miScalar	filter);	/* filter for lookup */
miBoolean mi_lookup_vector_texture_x(
                miVector*	rvector,	/* returned vector */
                miState* 	state,
                miTag		tex,		/* texture to look up */
                miVector* 	coord,		/* texture coordinate */
...
miBoolean mi_lookup_filter_color_texture(
                ...
                miState *	state,
                miTag		tex,		/* texture to look up */
                miTexfilter *	paras,		/* filter options */
                miMatrix	p2t);		/* pixel->texture transform */

miBoolean mi_lookup_filter_color_texture_x(
                miColor	* 	color,		/* returned color */
                miState	* 	state,
                miTag		tex,		/* texture image to look up */
                miTexfilter *	paras,		/* filter options */
                miMatrix	p2t,		/* pixel->texture transform */
                miScalar	filter);	/* the filter value to use */
...
miBoolean mi_exclusive_lightlist(
                ...
                miTag		**lights,	/* lightlist (array of tags) */
                miState 	*state);	/* state */
miBoolean mi_expand_lightlist(
                int		*n_lights,	/* number of lights */
                miTag		**lights,	/* lightlist (array of tags) */
                miState		*state);	/* state */
miBoolean mi_instance_lightlist(
                int		*n_lights,	/* number of inst. lights */
                miTag		**lights,	/* inst. light list */
...
miBoolean mi_tri_vectors(
                ...
                const miVector	**a,		/* returned vectors */
                const miVector	**b,
                const miVector	**c);
...
miBoolean mi_trace_eye(
                miColor 	*result,	/* result */
                miState 	*state, 	/* state */
                const miVector	*origin,	/* ray origin */
                const miVector	*direction);	/* ray direction */
...
miBoolean mi_trace_environment(
                miColor 	*result,	/* result */
                miState 	*state, 	/* state */
                const miVector	*direction);	/* direction */
miBoolean mi_trace_probe(
                miState 	*state, 	/* state */
                const miVector  *direction,	/* direction */
                const miVector 	*origin);	/* origin */
miBoolean mi_trace_probe_x(
                miState 	*state, 	/* state */
                const miVector  *direction,	/* direction */
                const miVector 	*origin,	/* origin */
                miUint		flag );		/* intersection flag */
...
void mi_opacity_set(
                ...
                const miColor  	*opacity); 	/* opacity to store */
...
miC_LINKAGE miBoolean mi_get_contour_line(
                miContour_endpoint *p1,
                miContour_endpoint *p2);

miC_LINKAGE void mi_add_contour_lines(
...
void mi_point_to_world(
                const miState *, miVector *, const miVector *);
...
typedef struct miIrrad_options {
  ...
  miUint	id;			/* id of fg points to use */
  miUint	finalgather_force;	/* 0==see inst/obj, 1==on, 2==off */
  miUint	finalgather_points_test;/* 0==see global flag, 1==on, 2==off */
 ...
 * defaults were necessary. Negative importance means: use the default one.
 */
#define miIRRAD_DEFAULT(irrad, state) do {                                  \
  ...
  (irrad)->id                    = 0;     /* no id by default    */         \
  (irrad)->finalgather_force     = 0;     /* see inst/obj by default */     \
  (irrad)->finalgather_points_test=0;     /* see global flag by default */  \
} while(0)
...
typedef struct miAmboccl_options {
  ...
  int		flags;		/* intersection flags (miTrace_flag) */
  miScalar	falloff;	/* falloff exponent                  */
} miAmboccl_options;


/*
 * Initialize values from the default options. Call this macro, then overwrite
 * defaults where necessary.
 */
#define miAMBOCCL_DEFAULT(ao, state) do {                                   \
  (ao)->size                  	 = sizeof(miAmboccl_options);               \
  (ao)->rays			 = -1;	  /* use string options */	    \
  (ao)->axis			 = state->normal;			    \
  (ao)->spread			 = -2;	  /* out of range, use default */   \
  (ao)->spread_exp		 = 1;	  /* lambertian */		    \
  (ao)->mindist			 = -1;	  /* use string options */	    \
  (ao)->maxdist			 = -1;	  /* use string options */	    \
  (ao)->cache_points		 = -1;	  /* use string options */	    \
  (ao)->flags			 = -1;	  /* use default */		    \
  (ao)->falloff			 = -1;	  /* use string options */	    \
} while(0)

/*****************************************************************************
 * computes and returns the ambient occlusion factor.
 * A value of 0 means completely unoccluded, while 1 means totally occluded
 */
...
/* all spectra contain in the first four float entries a color, the
 * CIE XYZ value of the spectrum. 
 */
#define miSPECTRUM_MAX_SIZE 74  

typedef float miSpectrum_para[miSPECTRUM_MAX_SIZE];
...
struct miSpectrum*	mi_spectrum_ensure	(struct miSpectrum* s);
...
void			mi_spectrum_reset	(struct miSpectrum *s);
...
void			mi_spectrum_rgb		(miColor	    *c,
						 const struct miSpectrum* s);
...
miScalar		mi_spectrum_value_get	(const struct miSpectrum* s,
						 miScalar   wavelen);
...
void			mi_spectrum_visible_range(miScalar* min_lambda, 
						  miScalar* max_lambda);

void			mi_spectrum_blackbody   (miSpectrum* bbody,
						 miScalar temperature,
						 miScalar intensity);
void			mi_spectrum_cie         (miSpectrum* spec, 
						 miScalar intensity,
						 const char* illuminant);
void			mi_spectrum_cie_sky	(miSpectrum* sky, 
						 miScalar temperature,
						 miScalar intensity);
...
#endif
#ifndef MI_PRODUCT_RAY
#define MI_PRODUCT_RAY
#endif
...
// Some compilers fall apart when they see a placement delete.
#if defined(__SUNPRO_CC)
#define MI_MEM_NO_PLACEMENT_DELETE
#endif
...
/// Allocator class implementation based on mi_mem_ functions.
class Allocator_impl_mi_mem
{
public:
    static void *amalloc(size_t sz)
    { return mi_mem_int_allocate_sizet(0, 0, sz); }

    static void *amalloc(const char* file, int line, size_t sz)
    { return mi_mem_int_allocate_sizet(file, line, sz); }

    static void afree(void *p)
    { mi_mem_int_release(0, 0, p); }

    static void afree(const char *file, int line, void *p)
    { mi_mem_int_release(file, line, p); }
};
...
public:
    ...
    void operator delete ( void* p, size_t)    { Impl::afree(p); }
    void operator delete[] ( void* p, size_t)  { Impl::afree(p); }

    /// The placement allocation function.
    void* operator new (size_t sz, void* a)  { return a; }

#ifndef MI_MEM_NO_PLACEMENT_DELETE
...
typedef Tallocatable< Allocator_impl_mi_mem >  Allocatable;
}} // MI::MEM
...
#ifdef MI_METASL_HEADERS
#include "shader_metasl.h"
#endif
#endif
